...
void MiscellaneousCommand::Execute() 
{
	...
	else if(state_ == 1)		
	{
		...
		flash::get_default_cg_producer(pCIIStrategy_->GetChannel(), false)->add(layer_, filename_, false, TEXT(""), xmlData2);
	}
}
...
void KeydataCommand::Execute() 
{
	...
	//TODO: Need to be checked for validity
		...
		flash::get_default_cg_producer(pCIIStrategy_->GetChannel(), false)->stop(layer_, 0);
	else if(state_ == 2)
		pCIIStrategy_->GetChannel()->stage()->clear(flash::cg_producer::DEFAULT_LAYER);
	else if(state_ == 3)
		flash::get_default_cg_producer(pCIIStrategy_->GetChannel(), false)->play(layer_);
}
