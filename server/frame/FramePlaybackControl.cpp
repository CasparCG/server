...
eventLoad_(FALSE, FALSE), eventRender_(FALSE, FALSE), eventStartPlayback_(FALSE, FALSE), eventPausePlayback_(FALSE, FALSE), eventStopPlayback_(FALSE, FALSE), eventStoppedPlayback_(FALSE, FALSE)
#ifdef ENABLE_MONITOR
, pMonitor_(0)
#endif
{
...
#ifdef ENABLE_MONITOR
	if(pMonitor_)
		pMonitor_->Inform(Monitor::STOPPED);
#endif
...
#else
		CG::FlashCGProxyPtr pNewCGProducer = CG::FlashCGProxy::Create();
#endif
		if(pNewCGProducer) {
