...
bool LoadCommand::Execute()
{
	...
	if(pMediaManager != 0)
	{
		...
		if(GetChannel()->Load(pFP, bLoop))
		{
			...
			GetChannel()->GetMonitor().Inform(LOAD, _parameters[0]);
			return true;
		}
		else
		{
...
bool LoadbgCommand::Execute()
{
	...
	if(pMediaManager != 0)
	{
		...
		if(GetChannel()->LoadBackground(pFP, transitionInfo, bLoop))
		{
			...
			GetChannel()->GetMonitor().Inform(LOADBG, _parameters[0]);
			return true;
		}
		else
		{
...
bool ClearCommand::Execute()
{
	...
	if(GetChannel()->Clear())
	{
		...
		GetChannel()->GetMonitor().Inform(CLEAR);
		return true;
	}
...
//	tstring data;			//_parameters[4] alt. _parameters[5]
	...
	if(GetApplication()->FindTemplate(fullFilename, &extension))
	{
		...
		GetChannel()->GetMonitor().Inform(CG_ADD, _parameters[2]);
	}
	else
	{
...
bool CGCommand::ExecuteClear() {
	...
	SetReplyString(TEXT("202 CG OK\r\n"));
	GetChannel()->GetMonitor().Inform(CG_CLEAR);
	return true;
}
