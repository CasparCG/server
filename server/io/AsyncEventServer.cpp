...
bool AsyncEventServer::Start() {
	...
	if(listenSocket == INVALID_SOCKET) {
		LOG << TEXT("Failed to create listenSocket");
		return false;
	}
	...
	if(!listenThread_.Start(this)) {
		LOG << TEXT("Failed to create ListenThread");
		return false;
	}

	LOG << TEXT("Listener successfully initialized");
	return true;
}
			...
			if(eventIndex == 0)	//stopEvent
				...
				if(networkEvents.lNetworkEvents & FD_ACCEPT) {
					if(networkEvents.iErrorCode[FD_ACCEPT_BIT] == 0)
						...
						LOG << LogLevel::Debug << TEXT("OnAccept (ErrorCode: ") << networkEvents.iErrorCode[FD_ACCEPT_BIT] << TEXT(")");
				...
				if(networkEvents.lNetworkEvents & FD_CLOSE) {
					if(networkEvents.iErrorCode[FD_CLOSE_BIT] == 0)
						...
						LOG << LogLevel::Debug << TEXT("OnClose (ErrorCode: ") << networkEvents.iErrorCode[FD_CLOSE_BIT] << TEXT(")");
				...
				if(networkEvents.lNetworkEvents & FD_READ) {
					if(networkEvents.iErrorCode[FD_READ_BIT] == 0)
						...
						LOG << LogLevel::Debug << TEXT("OnRead (ErrorCode: ") << networkEvents.iErrorCode[FD_READ_BIT] << TEXT(")");
				...
				if(networkEvents.lNetworkEvents & FD_WRITE) {
					if(networkEvents.iErrorCode[FD_WRITE_BIT] == 0)
						...
						LOG << LogLevel::Debug << TEXT("OnWrite (ErrorCode: ") << networkEvents.iErrorCode[FD_WRITE_BIT] << TEXT(")");
...
bool AsyncEventServer::OnUnhandledException(const std::exception& ex) throw() {
	...
	try 
	{
		LOG << TEXT("UNHANDLED EXCEPTION in TCPServers listeningthread. Message: ") << ex.what();
	}
	catch(...)
	{
		bDoRestart = false;
...
//	}
	...
	if(!listenThread_.Stop()) {
		LOG << TEXT("Wait for listenThread timed out.");
	}
...
bool AsyncEventServer::OnAccept(SocketInfoPtr& pSI) {
	...
	LOG << TEXT("Accepted connection from ") << pClientSocket->host_.c_str();

	return true;
}

bool ConvertMultiByteToWideChar(UINT codePage, char* pSource, int sourceLength, caspar::utils::DataBuffer<wchar_t>& wideBuffer, int& countLeftovers)
{
	if(codePage == CP_UTF8) {
		countLeftovers = 0;
		...
		if(pSource[sourceLength-1] & 0x80) {
			...
			int bytesToCheck = min(4, sourceLength);	//a sequence contains a maximum of 4 bytes
	...
	if(wideBufferCapacity > 0) 
	{
		...
		charsWritten = MultiByteToWideChar(codePage, 0, pSource, sourceBytesToProcess, wideBuffer.GetPtr(), wideBuffer.GetCapacity());
	}
...
bool AsyncEventServer::OnRead(SocketInfoPtr& pSI) {
	...
	while(recvResult != SOCKET_ERROR) {
		if(recvResult == 0) {
			LOG << TEXT("Client ") << pSI->host_.c_str() << TEXT(" disconnected");
		...
		if(pProtocolStrategy_ != 0) {
			...
			{
				LOG << TEXT("Read from ") << pSI->host_.c_str() << TEXT(" failed, could not convert command to UNICODE");
			}
		}
...
bool ConvertWideCharToMultiByte(UINT codePage, const std::wstring& wideString, caspar::utils::DataBuffer<char>& destBuffer)
{
	int bytesWritten = 0;
	...
	if(multibyteBufferCapacity > 0) 
	{
		...
		bytesWritten = WideCharToMultiByte(codePage, 0, wideString.c_str(), static_cast<int>(wideString.length()), destBuffer.GetPtr(), destBuffer.GetCapacity(), NULL, NULL);
	}
...
void AsyncEventServer::DoSend(SocketInfo& socketInfo) {
	...
	while(!socketInfo.sendQueue_.empty() || socketInfo.currentlySending_.GetLength() > 0) {
		if(socketInfo.currentlySending_.GetLength() == 0) {
			...
			{
				LOG << TEXT("Send to ") << socketInfo.host_.c_str() << TEXT(" failed, could not convert response to UTF-8");
			}
		...
		if(socketInfo.currentlySending_.GetLength() > 0) {
			...
			if(sentBytes == SOCKET_ERROR) {
				int errorCode = WSAGetLastError();
				if(errorCode == WSAEWOULDBLOCK) {
					LOG << LogLevel::Debug << TEXT("Send to ") << socketInfo.host_.c_str() << TEXT(" would block, sending later");
					break;
				}
				else {
				...
				if(sentBytes == bytesToSend) {
					if(sentBytes < 200)
						LOG << LogLevel::Verbose << TEXT("Sent ") << socketInfo.sendQueue_.front().c_str() << TEXT(" to ") << socketInfo.host_.c_str();
					else
						LOG << LogLevel::Verbose << TEXT("Sent more than 200 bytes to ") << socketInfo.host_.c_str();
					...
					LOG << LogLevel::Verbose << TEXT("Sent partial message to ") << socketInfo.host_.c_str();
				}
			}
		}
		else
...
void AsyncEventServer::OnClose(SocketInfoPtr& pSI) {
	LOG << TEXT("Client ") << pSI->host_.c_str() << TEXT(" was disconnected");
...
void AsyncEventServer::OnError(HANDLE waitEvent, int errorCode) {
	if(errorCode == WSAENETDOWN || errorCode == WSAECONNABORTED || errorCode == WSAECONNRESET || errorCode == WSAESHUTDOWN || errorCode == WSAETIMEDOUT || errorCode == WSAENOTCONN || errorCode == WSAENETRESET) {
		...
		if(socketInfoCollection_.FindSocketInfo(waitEvent, pSocketInfo)) {
			LOG << TEXT("Client ") << pSocketInfo->host_.c_str() << TEXT(" was disconnected, Errorcode ") << errorCode;
		}
...
void AsyncEventServer::LogSocketError(const TCHAR* pStr, int socketError) {
	...
	LOG << TEXT("Failed to ") << pStr << TEXT(" Errorcode: ") << socketError;
}
