...
Application::Application(const tstring& cmdline, HINSTANCE hInstance) :	   hInstance_(hInstance), logLevel_(2), logDir_(TEXT("_log")), 
																		   videoDir_(TEXT("content\\_media")),
																		   templateDir_(TEXT("content")),
																		   dataDir_(TEXT("content\\_data")),
...
//	MessageBox(NULL, TEXT("Now is the time to connect for remote debugging..."), TEXT("Debug"), MB_OK | MB_TOPMOST);
	...
	try
	{
		...
		LOG << TEXT("Found ") << videoDeviceCount << TEXT(" video cards.");
		...
		for(unsigned int i=0;i<videoChannels_.size();++i) {
			...
			MediaManagerPtr pMediaManager = GetColorMediaManager();

			if(pMediaManager != 0 && pChannel != 0) {
				MediaProducerPtr pFP = pMediaManager->CreateProducer(TEXT("#00000000"));
				if(!pChannel->Load(pFP, false))
					return false;
			}
		}
	}
...
ControllerPtr Application::CreateTCPController(HKEY hSubkey) {
	...
	return ControllerPtr(new caspar::IO::AsyncEventServer(port));
}
...
bool Application::LoadSettings10()
{
	...
	if(RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Sveriges Television AB\\Caspar CG\\1.0"), 0,KEY_READ, &hKey) == ERROR_SUCCESS)
	{
		...
		LOG << TEXT("No 1.0 settings defined. Using defaults.") << LogStream::Flush;
	}
