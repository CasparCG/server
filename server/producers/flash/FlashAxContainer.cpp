...
{
	tstring str(request);
	if(str.find(TEXT("DisplayedTemplate")) != tstring::npos)
	{
		ATLTRACE(_T("ShockwaveFlash::DisplayedTemplate\n"));
		bReadyToRender_ = true;
	}
	else if(str.find(TEXT("OnCommand")) != tstring::npos) {
		//this is how templatehost 1.8 reports that a command has been received
		LOG << utils::LogLevel::Verbose << TEXT("TEMPLATEHOST: ") << str;
		bCallSuccessful_ = true;
	}
	else if(str.find(TEXT("Activity")) != tstring::npos)
	{
		LOG << utils::LogLevel::Verbose << TEXT("TEMPLATEHOST: ") << str;

		//this is how templatehost 1.7 reports that a command has been received
		if(str.find(TEXT("Command recieved")) != tstring::npos)
			bCallSuccessful_ = true;

#ifdef ENABLE_MONITOR
		if(pFlashProducer_ != 0 && pFlashProducer_->pMonitor_) {
			//GetApplication()->RunTaskAsync();
			tstring::size_type pos = str.find(TEXT('@'));
			if(pos != tstring::npos)
...
#ifdef ENABLE_MONITOR
	else if(str.find(TEXT("OnNotify")) != tstring::npos)
	{
		LOG << utils::LogLevel::Verbose << TEXT("TEMPLATEHOST: ") << str;
...
#endif
	else if(str.find(TEXT("IsEmpty")) != tstring::npos)
	{
		ATLTRACE(_T("ShockwaveFlash::IsEmpty\n"));
		bIsEmpty_ = true;
		if(pFlashProducer_ != 0 && pFlashProducer_->emptyCallback)
			pFlashProducer_->emptyCallback();
	}
	else if(str.find(TEXT("OnError")) != tstring::npos)
	{
		LOG << "FLASHERROR: " << str;
	}
...
void FlashAxContainer::SetFormat(const caspar::FrameFormatDescription& fmtDesc) {
	if(m_spInPlaceObjectWindowless != 0)
	{
		m_rcPos.top = 0;
		m_rcPos.left = 0;
...
/*	const caspar::FrameFormatDescription& fmtDesc = caspar::FrameFormatDescription::FormatDescriptions[format_];
