...
struct OGLVideoConsumer::Implementation
{
	...
	struct OGLPlaybackStrategy: public IFramePlaybackStrategy
	{
		...
		void DisplayFrame(Frame* pFrame)
		{
			...
			if(pFrame == NULL || pFrame->ID() == lastFrameID_) // Potential problem is that if the HDC is invalidated by external application it will stay that way, (R.N), keep or remove?
				return;		
