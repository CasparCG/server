    ...
    void send_to_flash(const std::wstring& data)
    {
        if (!clock_loaded_) {
            core::frame_producer_dependencies dependencies(
                channel_->frame_factory(), channels_, format_repository_, channel_->video_format_desc(), producer_registry_, cg_registry_);
            cg_registry_
...
{
    auto context =
        spl::make_shared<command_context>(channels, format_repository, channel, cg_registry, producer_registry);
