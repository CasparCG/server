...
class AMCPCommandQueue
{
  public:
    ...
    std::future<std::wstring> QueueCommandBatch(std::vector<std::shared_ptr<AMCPCommand>> cmds);

  private:
    std::wstring Execute(std::vector<std::shared_ptr<AMCPCommand>> cmds) const;
