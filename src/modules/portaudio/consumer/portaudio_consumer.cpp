  ...
  private:
    ...
    void perform_auto_tune()
    {
        ...
        if (std::abs(avg_error_ms) >= 2 && stddev_ms < 30) {
            ...
            // 75% correction per cycle to avoid oscillation; subtract to correct direction
            ...
            CASPAR_LOG(info) << L"PortAudio auto-tune: " << old_compensation << L"ms -> " << new_compensation
                             << L"ms (error=" << avg_error_ms << L"ms, adjustment=" << adjustment << L"ms)";
        } else if (stddev_ms >= 30) {
            CASPAR_LOG(warning) << L"PortAudio auto-tune skipped: timing inconsistent (StdDev=" << stddev_ms << L"ms)";
        } else {
            CASPAR_LOG(debug) << L"PortAudio auto-tune: timing good (" << avg_error_ms << L"ms)";
        }
