...
#if defined(_WIN32)
#define GET_PROCADDR_FOR_FUNC(name, module)                                                                            \
    {                                                                                                                  \
        name = (pFunc_##name)GetProcAddress(reinterpret_cast<HMODULE>(module), #name);                                 \
        if (!name) {                                                                                                   \
            return false;                                                                                              \
        }                                                                                                              \
    }
...
#define GET_PROCADDR_FOR_FUNC(name, module)                                                                            \
    {                                                                                                                  \
        name = (pFunc_##name)dlsym(module, #name);                                                                     \
        if (!name) {                                                                                                   \
            return false;                                                                                              \
        }                                                                                                              \
    }
...
bvc_wrapper::bvc_wrapper()
{
    if (!init_function_pointers())
        CASPAR_THROW_EXCEPTION(not_supported()
                               << msg_info("Bluefish drivers not found.  Unable to init Funcion Pointers"));
...
bool bvc_wrapper::init_function_pointers()
{
    ...
    char* libraryPath("/Library/Frameworks/BlueVelvetC.framework"); // full path may not be required, OSX might check in
                                                                    // /l/f by default - MUST TEST!
...
BLUE_UINT32 bvc_wrapper::attach(int iDeviceId) { return bfcAttach(bvc_.get(), iDeviceId); }

BLUE_UINT32 bvc_wrapper::detach() { return bfcDetach(bvc_.get()); }
...
BLUE_UINT32 bvc_wrapper::enumerate(int& iDevices) { return bfcEnumerate(bvc_.get(), iDevices); }
...
BLUE_UINT32 bvc_wrapper::system_buffer_write(unsigned char* pPixels,
...
BLUE_UINT32 bvc_wrapper::system_buffer_read(unsigned char* pPixels,
                                            unsigned long  ulSize,
                                            unsigned long  ulBufferID,
                                            unsigned long  ulOffset)
{
...
BLUE_UINT32 bvc_wrapper::encode_hanc_frame(unsigned int             nCardType,
...
BLUE_UINT32 bvc_wrapper::get_frame_info_for_video_mode(const unsigned int nVideoMode,
                                                       unsigned int&      nWidth,
...
BLUE_UINT32 bvc_wrapper::get_bytes_per_frame(EVideoMode    nVideoMode,
...
EVideoMode vid_fmt_from_video_format(const core::video_format& fmt)
{
    switch (fmt) {
        ...
        case core::video_format::x576p2500:
            return VID_FMT_INVALID; // not supported
...
EVideoMode get_video_mode(bvc_wrapper& blue, const core::video_format_desc& format_desc)
{
    ...
    if ((unsigned int)vid_fmt >= invalid_fmt)
        CASPAR_THROW_EXCEPTION(not_supported()
                               << msg_info(L"video-mode not supported - Outside of valid range: " + format_desc.name));
