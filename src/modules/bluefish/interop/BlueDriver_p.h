...
typedef enum _EBlueCardProperty
{
	...
	FORCE_SD_VBI_OUTPUT_BUFFER_TO_V210 = 134,	/* this card property forces the VBI output buffer to V210 memory format in SD video modes (default for HD video modes) so that it can handle 10 bit VANC packets.
...
#define OUTPUT_VPID_SET_BYTES(value, byte1, byte2, byte3, byte4)	(value = ((value & 0xFFFFFFFF00000000ULL) | ((((BLUE_UINT64)(byte4 & 0xFF)) << 24) | ((byte3 & 0xFF) << 16) | ((byte2 & 0xFF) << 8) | (byte1 & 0xFF))))
