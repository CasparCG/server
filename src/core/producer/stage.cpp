  ...
  public:
    ...
    std::future<frame_producer_and_attrs> foreground(int index)
    {
        return executor_.begin_invoke(
            [=, this]() -> frame_producer_and_attrs { return get_layer(index).foreground(); });
    }

    std::future<frame_producer_and_attrs> background(int index)
    {
        return executor_.begin_invoke(
            [=, this]() -> frame_producer_and_attrs { return get_layer(index).background(); });
    }

    std::future<std::wstring> call(int index, const std::vector<std::wstring>& params)
    {
        return flatten(
            executor_.begin_invoke([=, this] { return get_layer(index).foreground().producer->call(params).share(); }));
    }
    std::future<std::wstring> callbg(int index, const std::vector<std::wstring>& params)
    {
        return flatten(
            executor_.begin_invoke([=, this] { return get_layer(index).background().producer->call(params).share(); }));
    }
...
{
    return executor_.begin_invoke([=, this]() -> frame_producer_and_attrs { return stage_->foreground(index).get(); });
}
std::future<frame_producer_and_attrs> stage_delayed::background(int index)
{
    return executor_.begin_invoke([=, this]() -> frame_producer_and_attrs { return stage_->background(index).get(); });
}
