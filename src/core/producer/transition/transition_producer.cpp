...
class transition_producer : public frame_producer
{
  ...
  public:
    ...
    [[nodiscard]] draw_frame compose(const core::video_field field, int nb_samples) const
    {
        ...
        auto get_src_frame = [&]() {
            auto f = src_producer_.producer->receive(field, nb_samples);
            if (!f)
                f = src_producer_.producer->last_frame(field);
            return draw_frame::push(std::move(f));
        };
        auto get_dst_frame = [&]() {
            auto f = dst_producer_.producer->receive(field, nb_samples);
            if (!f)
                f = dst_producer_.producer->last_frame(field);
            return draw_frame::push(std::move(f));
        };
