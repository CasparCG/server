...
typedef std::function<boost::optional<interaction_target>(double x, double y)> collission_detector;
...
class interaction_aggregator
{
  ...
  public:
    ...
    {
        if (!events_.empty() && is<mouse_move_event>(event) && is<mouse_move_event>(events_.back())) {
            events_.back() = event;
        } else {
            events_.push(event);
        }
    ...
    void translate_and_send(const interaction_event::ptr& event)
    {
        if (is<position_event>(event)) {
            ...
            if (target)
                target->second->on_interaction(pos_event->translate(target->first));
        }
    }
};
