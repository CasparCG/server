...
mutable_frame::mutable_frame(const void*                      tag,
                             ...
                             commit_t                         commit,
                             frame_side_data_in_queue         side_data)
    : impl_(new impl(tag, std::move(image_data), std::move(audio_data), desc, std::move(commit), std::move(side_data)))
{
}
...
const_frame::const_frame(const void*                            tag,
                         ...
                         std::shared_ptr<core::texture>         texture,
                         frame_side_data_in_queue               side_data)
    : impl_(new impl(tag, std::move(image_data), std::move(audio_data), desc, std::move(texture), std::move(side_data)))
{
}
...
const_frame                      const_frame::with_tag(const void* new_tag) const
{
    ...
    auto                                   new_frame       = const_frame(
        new_tag, std::move(image_data_copy), impl_->audio_data_, impl_->desc_, impl_->texture_, impl_->side_data_);
