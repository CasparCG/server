...
export function registerProducerCommands(
    ...
    async function loadBgCommand(
        ...
        channelIndex: number,
        layerIndex0: number | null
    ): Promise<AMCPCommandCommitFunction> {
    ...
    channelCommands.set("LOAD", {
        func: async (_context, command, channelIndex, layerIndex0) => {
            const layerIndex = layerIndex0 ?? DEFAULT_LAYER_INDEX;
