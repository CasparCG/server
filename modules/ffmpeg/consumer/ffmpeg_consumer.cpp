...
public:
	...
	std::shared_ptr<AVFrame> convert_video_frame(const safe_ptr<core::data_frame>& frame, AVCodecContext* c)
	{
		if(!sws_) 
		{
	...
	std::shared_ptr<AVPacket> encode_video_frame(const safe_ptr<core::data_frame>& frame)
	{ 
	...
	std::shared_ptr<AVPacket> encode_audio_frame(const safe_ptr<core::data_frame>& frame)
	{			
