...
{	
	...
	boost::circular_buffer<CComPtr<IDeckLinkVideoInputFrame>>	video_delay_;
	boost::circular_buffer<CComPtr<IDeckLinkAudioInputPacket>>	audio_delay_;
...
public:
	decklink_producer(const core::video_format_desc& format_desc, size_t device_index, const safe_ptr<core::frame_factory>& frame_factory, const std::wstring& filter, std::size_t buffer_depth)
		: decklink_(get_device2(device_index))
		, input_(decklink_)
	...
	{		
		video_delay_.set_capacity(1);
		audio_delay_.set_capacity(1);
	...
	{	
		...
		video_delay_.push_back(video);
		video = video_delay_.front();
		
		audio_delay_.push_back(audio);
		audio = audio_delay_.front();
	...
	{
		static boost::wregex video_delay_expr(L"DELAY VIDEO (?<DELAY>\\d+)");
		static boost::wregex audio_delay_expr(L"DELAY AUDIO (?<DELAY>\\d+)");

		boost::wsmatch what;

		if(boost::regex_match(params, what, video_delay_expr))
		{
			set_video_delay(boost::lexical_cast<int>(what["DELAY"].str()));
		}
		else if(boost::regex_match(params, what, audio_delay_expr))
		{			
			set_audio_delay(boost::lexical_cast<int>(what["DELAY"].str()));
		}

		boost::promise<std::wstring> p;
		p.set_value(L"");
		return p.get_future();
	}

	void set_video_delay(int delay)
	{
		video_delay_.set_capacity(delay+1);
	}

	void set_audio_delay(int delay)
	{
		audio_delay_.set_capacity(delay+1);
	}
...
public:
	...
	{
		auto frame = context_->receive(hints);
