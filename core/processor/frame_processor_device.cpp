...
{	
	...
	boost::unique_future<safe_ptr<const read_frame>> process(safe_ptr<draw_frame>&& frame)
	{			
		return executor_.begin_invoke([=]() -> safe_ptr<const read_frame>
		{		
...
boost::unique_future<safe_ptr<const read_frame>> frame_processor_device::process(safe_ptr<draw_frame>&& frame){return impl_->process(std::move(frame));}
