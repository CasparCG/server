...
struct gpu_frame_shader::implementation
{
	implementation(const frame_format_desc& format_desc) 
		: current_(pixel_format::invalid_pixel_format), format_desc_(format_desc)
	{
		std::string common = 
...
//		set_size(desc);
	...
	//void set_size(gpu_frame_desc& desc)
	//{
	//	for(int n = 0; n < 4; ++n)
	//	{
	...
	std::unordered_map<pixel_format, shader_program> shaders_;
};

gpu_frame_shader::gpu_frame_shader(const frame_format_desc& format_desc) : impl_(new implementation(format_desc)){}
void gpu_frame_shader::use(gpu_frame_desc& desc){impl_->use(desc);}
