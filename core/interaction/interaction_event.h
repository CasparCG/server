...
struct interaction_event
		: public spl::enable_shared_from_this<interaction_event>
{
...
struct position_event : public interaction_event
{
	...
	virtual interaction_event::ptr translate(
			const frame_transform& transform) const
	{
...
struct mouse_button_event : public position_event
{
	...
	mouse_button_event(
			int source_id, double x, double y, int button, bool pressed)
		: position_event(source_id, x, y)
		, button(button)
		, pressed(pressed)
	{
...
protected:
	virtual interaction_event::ptr clone(
			double new_x, double new_y) const override
	{
		return spl::make_shared<mouse_button_event>(
				source_id, new_x, new_y, button, pressed);
	}
};
