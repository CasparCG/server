...
public:
	...
	void operator()(spl::shared_ptr<const data_frame> input_frame, const core::video_format_desc& format_desc)
	{
		...
		{			
			...
			frames_.set_capacity(std::max(2, minmax.second - minmax.first) + 1); // std::max(1, x) since we want to guarantee some pipeline depth for asycnhronous mixer read-back.
