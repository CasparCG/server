...
std::array<GLubyte, 32*32> upper_pattern = {{
...
std::array<GLubyte, 32*32> lower_pattern = {{
	...
	void draw(draw_params&& params)
	{
		...
		{		
			...
			for(int n = 0; n < params.textures.size(); ++n)
				ogl_->bind(*params.textures[n], n);

			if(params.local_key)
				ogl_->bind(*params.local_key, texture_id::local_key);
		
			if(params.layer_key)
				ogl_->bind(*params.layer_key, texture_id::layer_key);
