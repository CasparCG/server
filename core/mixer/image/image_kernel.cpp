...
__declspec(align(16)) std::array<GLubyte, 32*32> lower_pattern = {{
...
__declspec(align(16)) std::array<GLubyte, 32*32> progressive_pattern = {{
...
{	
	...
	void draw(draw_params&& params)
	{
		...
		if(params.transform.field_mode == core::field_mode::upper)
			...
			ogl_->stipple_pattern(progressive_pattern);
