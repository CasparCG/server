...
struct device::impl : public std::enable_shared_from_this<impl>
{
        ...
        , thread_([this] { context_.run(); })
	{
        ...
        caspar::dispatch_async(context_, [=]
		{
	...
	std::wstring version()
	{
		try
		{
			return caspar::dispatch_async(context_, [=]
			{
    ...
    std::shared_ptr<texture> create_texture(int width, int height, int stride, bool clear)
	{
		...
		// TODO (fix) This should be callable out of context.
		...
		auto ptr = tex.get();
        return std::shared_ptr<texture>(ptr, [=, tex = std::move(tex), self = shared_from_this()](texture*) mutable
        {
	...
	{
		return caspar::dispatch_async(context_, [=, source = std::move(source)]
		{
			std::shared_ptr<buffer> buf;

			auto tmp_buf = source.template storage<std::shared_ptr<buffer>>();
			if (tmp_buf) {
				buf = *tmp_buf;
			} else {
	...
	{
		return spawn_async(context_, [=, source = std::move(source)](yield_context yield)
		{
			...
			for (auto n = 0; true; ++n) {
				auto wait = glClientWaitSync(fence, GL_SYNC_FLUSH_COMMANDS_BIT, 0);
				if (wait == GL_ALREADY_SIGNALED || wait == GL_CONDITION_SATISFIED) {
					break;
				}
				// TODO GL_WAIT_FAILED
				deadline_timer timer(context_, boost::posix_time::milliseconds(2));
