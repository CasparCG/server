...
bool operator==(const item& lhs, const item& rhs) { return lhs.frame == rhs.frame && lhs.transform == rhs.transform; }

bool operator!=(const item& lhs, const item& rhs) { return !(lhs == rhs); }
  ...
  private:
    ...
    void convert(std::vector<item>& source_items, int width, int height)
    {
        ...
        tbb::parallel_for_each(buffers.begin(), buffers.end(), [&](const std::shared_ptr<AVFrame>& in_frame) {
            auto out_frame    = std::shared_ptr<AVFrame>(av_frame_alloc(), [](AVFrame* ptr) { av_frame_free(&ptr); });
            ...
            if (!pool.try_pop(sws_device)) {
                double param;
                sws_device.reset(sws_getContext(in_frame->width,
                                                in_frame->height,
...
struct image_mixer::impl : boost::noncopyable
{
  ...
  public:
    ...
    void visit(const core::const_frame& frame)
    {
        ...
        auto in_frame = std::shared_ptr<AVFrame>(av_frame_alloc(), [frame](AVFrame* ptr) { av_frame_free(&ptr); });
